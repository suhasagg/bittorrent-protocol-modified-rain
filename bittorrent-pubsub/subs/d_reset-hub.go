...
type LegacyBittorrentResetHub struct {
	mutex    *sync.RWMutex
	registry map[pubsub.Subscriber]bool
}

// New returns new hub instance. hub is goroutine safe.
func NewLegacyBittorrentResetHub() (result pubsub.ResetHub) {
...
func (h *LegacyBittorrentResetHub) CloseSubscriber(subscriber pubsub.Subscriber) {
	h.mutex.Lock()
...
func (h *LegacyBittorentResetHub) Publish(message pubsub.Message) int {
	h.mutex.Lock()
...
func (h *LegacyBittorrentResetHub) Subscribe(topics ...string) pubsub.Subscriber {
	var result pubsub.Subscriber
	if len(topics) > 0 {
		result = newBittorrentSubscriber(h, topics[0])
	} else {
		result = newBittorrentSubscriber(h, "")
	}
...
func (h *LegacyBittorrentResetHub) Reset() {
	h.mutex.Lock()
