...
import (
	...
	"github.com/bittorrent/log"
	"github.com/phonkee/go-pubsub"
	"sync"
)

type bittorrentWSJsonResult struct {
	Result       json.RawMessage `json:"result"`
	Subscription string          `json:"subscription"`
}

type bittorrentWSJsonRpcResponse struct {
...
func newWsSubscriber(hub pubsub.ResetHub, conn *websocket.Conn, id string) *wsSubscriber {
	sf := func(msg pubsub.Message) {
		resp := bittorrentWSJsonRpcResponse{
			Params:  bittorrentWSJsonResult{msg.Body(), id},
			Version: "2.0",
			Method:  "bittorrent_subscription",
		}
