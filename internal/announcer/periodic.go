...
type PeriodicalAnnouncer struct {
	...
	seeders       int //Definition
	leechers      int //Definition
	warningMsg    string
	lastError     *AnnounceError
	log           logger.Logger
	completedC    chan struct{} //Algorithm completed
	newPeers      chan []*net.TCPAddr
	backoff       &backoff.BackOff //Exponential backoff - delay (announcement requests - error prone)
	getTorrent    func() tracker.Torrent
	lastAnnounce  time.Time
	nextAnnounce  time.Time
	HasAnnounced  bool
	responseC     chan *tracker.AnnounceResponse
	errC          chan error //Error co-ordination+

	closeC chan struct{} //Close channel
	doneC  chan struct{} //Done channel

	needMorePeers  bool
	mNeedMorePeers sync.RWMutex //Must be getting used - multiple go routines (need more peers) - need more peers (one at a time (atomic) (attempt the next one) thread safe
...
func NewPeriodicalAnnouncer(trk tracker.Tracker, numWant int, minInterval time.Duration, getTorrent func() tracker.Torrent, completedC chan struct{}, newPeers chan []*net.TCPAddr, l logger.Logger) *PeriodicalAnnouncer {
	return &PeriodicalAnnouncer{
		...
		//Exponential backoff - standard defaults - compare with go concurrency patterns
...
func (a *PeriodicalAnnouncer) Run() {
	...
	for {
		...
		case resp := <-a.responseC:
            //Response is written and response parameters are parsed
			//Response is received, url is working, set to true 
			a.status = Working
			//Total number of peers - Seeders + Leechers available in tracker annoucement (for need for more peers) response
			//Seeders(having full file pieces, available for download permanently)
			//Leechers(upload file pieces only at the time of downloading )
			...
			a.interval = resp.Interval
			 //Used as normal periodic announcement
			if resp.MinInterval > 0 {
				a.minInterval = resp.MinInterval
				//Used if need more peers is true
			}

			a.HasAnnounced = true
			a.lastError = nil
			//Response might have been received after exponential back off retry, after successful response backofff parameters are reset
			a.backoff.Reset()
			//Get Next interval is derived based on immediate peer requirement, use minimum or periodic interval
		...
		case err := <-a.errC:
			//Error is pulled from error announcement channel
			a.status = NotWorking
			...
			//In case of error, request is tried with exponential backoff 
			interval := a.getNextIntervalFromError(a.lastError)
			resetTimer(interval)
		case <-a.needMorePeersC:
			//In case tracker url is not working, it will try an alternate url
