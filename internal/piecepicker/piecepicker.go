...
type PiecePicker struct {
	...
	//Total Piece array
	pieces []myPiece
	//Total Piece by availability for a peer
	piecesByAvailability []*myPiece
	//Total Piece which are stalled for that peer
...
type myPiece struct {
	*piece.Piece
	...
	//Piece is requested from this peers
	Requested peerset.PeerSet
	//Peers marked as snub state for this piece
	Snubbed peerset.PeerSet
    //Some peers may oscillate between snub state and normal state and might get low peer priority
	//Peers marked as choke state for this piece
	Choked peerset.PeerSet
    //Some peers may oscillate between choked state and normal state and might get low peer priority
...
func (p *myPiece) RunningDownloads() int {
	//Requested Pieces for which download is active from a peer - stalled pieces --> Total Number of Active Downloads
...
func (p *myPiece) StalledDownloads() int {
	//Calculate total  aomunt of stalled downloads - snubbed + choked
...
func New(pieces []piece.Piece, maxDuplicateDownload int, webseedSources []*webseedsource.WebseedSource) *PiecePicker {
	...
	return &PiecePicker{
		//Pieces specific to a torrent
		pieces: ps,
		//Piece array which is available for the peer
		//Does not have status for that pieces
		piecesByAvailability: sps,
		//Pieces are stalled, cannot be downloaded from this peer
...
func (p *PiecePicker) Available() uint32 {
	//Number of pieces available for the peer
	return p.available
}
...
func (p *PiecePicker) RequestedPeers(i uint32) []*peer.Peer {
	//Number of peers which are available for that piece
...
func (p *PiecePicker) HandleAllowedFast(pe *peer.Peer, i uint32) {
	//For a particular peer, this piece can be downloaded at faster rate
...
func (p *PiecePicker) HandleSnubbed(pe *peer.Peer, i uint32) {
	...
	//For this piece, peer is marked as snub - slow download
...
// At this point only peer information is available, information for piece is not provided - ? is this specific file download
func (p *PiecePicker) PickFor(pe *peer.Peer) (pp *piece.Piece, allowedFast bool) {
	//Pieces which have already been allocated to other peers, how are they getting distinguished
