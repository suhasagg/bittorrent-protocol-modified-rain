...
func (p *PeerWriter) queueMessage(msg peerprotocol.Message) {
	...
	case peerprotocol.ChokeMessage:
		//All the queued piece messages are cancelled as peer is choking and cannot provide download
		p.cancelQueuedPieceMessages()
	case Piece:
		// Reject request if peer queued to many requests
		// If queue is already full, piece message cannot be processed and piece message is rejected
		...
		//Peer current queued request counter is increased
		p.currentQueuedRequests++
	}
...
func (p *PeerWriter) cancelQueuedPieceMessages() {
	var next *list.Elementi
	//In case peer is choking, queue is made empty as it cannot process more messages
...
func (p *PeerWriter) cancelRequest(cm peerprotocol.CancelMessage) {
	 //Cancel a specific piece, if cancel piece message is received from peer
