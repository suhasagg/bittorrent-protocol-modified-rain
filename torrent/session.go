...
type Session struct {
	...
	log            logger.Logger
	extensions     [8]byte
	dht            *dht.DHT //Distributed hash table
	rpc            *rpcServer
	trackerManager *trackermanager.TrackerManager
	ram            *resourcemanager.ResourceManager
	pieceCache     *piececache.Cache //File piece Algorithm
	webseedClient  http.Client
	createdAt      time.Time
	semWrite       *semaphore.Semaphore //Concurrency control
	metrics        *sessionMetrics
	bucketDownload *ratelimit.Bucket //Rate limit for download
	bucketUpload   *ratelimit.Bucket //Rate limit for upload
	closeC         chan struct{}
...
//Torrent tracker is executed from browser or some node execute this tracker
