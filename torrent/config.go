...
type Config struct {
	...
	MaxPieces uint32 //part of File piece Algorithm - maximum number of pieces - for efficient storage and transfers (tradeoff)
	...
	// Start torrent automatically if it was running when previous session was closed. (torrent session was active, interrupted resume from the checkpoint)
	...
	// Different type of announcement in context of trackers
	...
	MaxRequestsIn int
	// Query for a piece, further blocks for that peice
